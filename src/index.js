...
const resultContainerGuideline = document.querySelector(".result-container__guideline");
const resultContainerError = document.querySelector(".result-container__error");
const scoreContainerOverall = document.querySelector(".score-container__overall");
const scoreContainerCategories = document.querySelector(".score-container__categories ");
...
const getDatas = async () => {
  //Fetch API
  const getScores = await fetch(`https://api.teleport.org/api/urban_areas/slug:${city}/scores/`);
  const scoresData = await getScores.json();

  const getImage = await fetch(`https://api.teleport.org/api/urban_areas/slug:${city}/images/`);
  ...
  if (getScores.status != 404) {
    ...
    scoresData.categories.forEach((x) => {
      scoreContainerCategories.insertAdjacentHTML(
        "afterbegin",
        `<h3><strong>${x.name}</strong>:<br> ${x.score_out_of_10.toFixed(1)} / 10</h3><br>`
      );
    });
